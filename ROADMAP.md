...
(`icon.png`, `wordmark.png`) and every raster under `app/static/` is now generated from it by
...
Shipped: `logo.png` 512 transparent, `favicon.png` 256, `favicon.ico` 16/32/48,
`apple-touch-icon.png` 180, `icon-192`/`icon-512` for the manifest, and `wordmark.png` 1600×266
(available but not yet used — the header still pairs the mark with an `<h1>`). Added
`site.webmanifest`, `theme-color`, and an `application/manifest+json` MIME registration in
`main.py` because `.webmanifest` is not in Python's default table. All four traps below are
closed: `VERSION` bumped to 1.5.1 for the `?v=` cache-bust, the logo now carries alpha, and the
circular crop is gone from both templates.
